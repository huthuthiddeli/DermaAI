...
class GaussianProcess:
    ...
    def train(self):
        ...
        joblib.dump(self.gpc, self.model_save_path + 'trained_gpc.joblib')
        print('Trained and dumped model: ', self.gpc.__class__.__name__)

    def predict(self):
        try:
            check_is_fitted(self.gpc)
        except:
            if os.path.isfile(self.model_save_path + 'trained_gpc.joblib'):
                self.gpc = joblib.load(self.model_save_path + 'trained_gpc.joblib')
        ...
        plt.title(self.gpc.__class__.__name__)
        plt.show()

        print(
            f"Classification report for classifier {self.gpc}:\n"
