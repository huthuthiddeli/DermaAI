import { predictionModel } from '#models/predictionCollection'
...
@inject()
export class PredictionProviderService {
  constructor() {}
  ...
  public async savePrediction(ctx: HttpContext) {
    try {
      ...
      let foundUser = await this.findOneDB(await hash.make(data.password), data.email)
